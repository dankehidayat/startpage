...
export default function Home() {
  ...
  // Close side drawer on outside click or Escape
  useEffect(() => {
    if (!drawerOpen) return;
    const onPointer = (e: MouseEvent) => {
      if (!document.getElementById("side-drawer")?.contains(e.target as Node)) {
        setDrawerOpen(false);
      }
    };
  ...
  return (
    <>
      ...
      {/* Side control drawer */}
      <div
        className={`side-drawer ${drawerOpen ? "side-drawer-open" : ""}`}
        id="side-drawer"
      >
        <button
        ...
        <div className="side-drawer-panel" id="side-drawer-panel">
          <button
            onClick={(e) => {
              e.stopPropagation();
              setDrawerOpen(false);
